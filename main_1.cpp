...
size_t dummy = 0; //to store dummy weights

vector<vector<vector<size_t>>> childPosScore;  //child1,child2.., score
...
void addWords(const vector<string> words, int score){  //max word length = 3
    ...
    if(words.size() > 1){
        ...
        childPosScore[parent_pos][childPosScore[parent_pos].size()-1].push_back(size_t(score));
...
vector<size_t> get_predictions(const vector<string> words){  //results: wordpos1,score1, wordpos2,score2 ..
    ...
    if(parent_pos >= parentWords.size()){
        ...
        if( wordListPos >= wordList.size()){
            ...
            if(words.size() > 1) {
                ...
                if (isDebug)
                    cout << "added new parent to parent list" << endl;
                ...
                childPosScore[childPosScore.size() - 1][0].push_back(10); //dummy score
            }
        }
    ...
    if(childPosScore.size() <= parent_pos) {
        ...
        if(words.size() > 1){
            ...
            for(int i = 1; i < words.size(); i++){
                ...
                childPosScore[parent_pos][0].push_back(dummy);
                if(isDebug)
    ...
    for(int i = 0; i < childPosScore[parent_pos].size(); i++){
        ...
        if(words.size()==1){
            ...
            results.push_back(childPosScore[parent_pos][i][0]); //word position
        ...
        for(int j = 0; j < childPosScore[parent_pos][i].size()-1; j++){
            //cout << "j: " << j << endl;
            if( j == words.size()-1 ){
                //cout << "result found" << endl;
                results.push_back(childPosScore[parent_pos][i][j+1]); //word position
                results.push_back(childPosScore[parent_pos][i][childPosScore[parent_pos][i].size()-1]); //score
                break;
            }
            ...
            if(strcmp(childWord.c_str(), words[j+1].c_str()) == 0) {
                ...
                if(j == words.size()-1 && childPosScore[parent_pos][i].size() == words.size())
                    return results;
                continue;
            }
            else
...
void queryFor(vector<string> query, vector<string> &output){
    ...
    vector<size_t> results = get_predictions(query);
